...
// sunhe {
static ngx_int_t ngx_http_mogilefs_add_etag(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source);
static ngx_int_t ngx_http_mogilefs_del_input_header(ngx_http_request_t *r, const u_char *key);
static ngx_int_t ngx_http_mogilefs_if_none_match(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source);
static ngx_int_t ngx_http_mogilefs_if_range(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source);
// sunhe }
    ...
    ngx_http_upstream_t *u, ngx_str_t *line)
{
    ...
    ngx_int_t                        rc;
// sunhe {
    ngx_int_t                        if_none_match = 0;
    ngx_int_t                        if_range = 0;
// sunhe }
...
//sunhe {
        if_none_match = ngx_http_mogilefs_if_none_match(r, source);
        if_range = ngx_http_mogilefs_if_range(r, source);
        if (if_none_match < 2 || if_range) {
// sunhe }
...
// sunhe {
        ...
        ngx_http_mogilefs_add_etag(r, source);
    }
    r->headers_out.content_length_n = 0;
    if (if_none_match == 2) {
        u->headers_in.status_n = NGX_HTTP_NOT_MODIFIED;
        u->state->status = NGX_HTTP_NOT_MODIFIED;
    } else {
        if (if_range == 2) {
            u->headers_in.status_n = NGX_HTTP_PARTIAL_CONTENT;
            u->state->status = NGX_HTTP_PARTIAL_CONTENT;
        } else {
            u->headers_in.status_n = 200;
            u->state->status = 200;
            if (if_range == 1) {
                ngx_http_mogilefs_del_input_header(r, (const u_char *)"If-Range");
                ngx_http_mogilefs_del_input_header(r, (const u_char *)"Range");
...
// sunhe {
static ngx_int_t
ngx_http_mogilefs_get_fid(ngx_http_mogilefs_src_t *source, u_char *fid)
{
    u_char              *fid_start;
    u_char              *fid_last;
    size_t              fid_len;
    ...
    *fid = '\0';
    fid_last = (u_char *)ngx_strstr((char *)source->path.data, ".fid");
    if (fid_last == NULL) {
        return NGX_ERROR;
    }
    fid_last--;
    if (fid_last < source->path.data) {
        return NGX_ERROR;
    }
    fid_start = fid_last;
    while (fid_start > source->path.data) {
        if (*fid_start == '/') {
            fid_start++;
            break;
        }
    ...
    fid_len = fid_last-fid_start+1;
    ngx_cpystrn(fid, fid_start, fid_len+1);

    return NGX_OK;
}

static ngx_int_t
ngx_http_mogilefs_get_header(ngx_http_request_t *r, const u_char *key, u_char *value)
{
    ...
    *value = '\0';
    ...
    for (i=0; i<r->headers_in.headers.part.nelts; i++) {
        if (ngx_strcmp(headers[i].key.data, key) == 0) {
            ngx_cpystrn(value, headers[i].value.data, headers[i].value.len+1);
            return NGX_OK;
        }
    } 
...
static u_char *
ngx_http_mogilefs_strip_etag(u_char *s)
{
    u_char              *p;
...
static ngx_int_t
ngx_http_mogilefs_etag_cmp(u_char *s1, u_char *s2)
{
    s1 = ngx_http_mogilefs_strip_etag(s1);
    s2 = ngx_http_mogilefs_strip_etag(s2);

    return ngx_strcmp(s1, s2);
}

// 0 is no this head; 1 is error; 2 is matched;
static ngx_int_t
ngx_http_mogilefs_if_none_match(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source)
{
    u_char              value[128];
    u_char              fid[32];
    const char          *delim = " ,";
    ...
    if (ngx_http_mogilefs_get_fid(source, fid) == NGX_ERROR) {
        return 0;
    }
    if (ngx_http_mogilefs_get_header(r, (const u_char *)"If-None-Match", value) == NGX_OK) {
        for (token = (u_char *)strtok((char *)value, delim); token; token = (u_char *)strtok(NULL, delim)) {
            if (ngx_http_mogilefs_etag_cmp(token, fid) == 0) {
                return 2;
            }
        }
        return 1;
...
// 0 is no this head; 1 is error; 2 is matched;
static ngx_int_t
ngx_http_mogilefs_if_range(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source)
{
    u_char              fid[32];
    u_char              if_range_value[32];

    if (ngx_http_mogilefs_get_fid(source, fid) == NGX_ERROR) {
        return 0;
    }
    if (r->headers_in.if_range) {
        ngx_cpystrn(if_range_value, r->headers_in.if_range->value.data, r->headers_in.if_range->value.len+1);
        if (ngx_http_mogilefs_etag_cmp(if_range_value, fid) == 0) {
            return 2;
        }
        return 1;
    }
...
static ngx_int_t 
ngx_http_mogilefs_add_etag(ngx_http_request_t *r, ngx_http_mogilefs_src_t *source)
{
    ngx_table_elt_t     *h;
    u_char              *etag_value;
    u_char              fid[32];
    ...
    if(source == NULL || source->path.len == 0) {
        return NGX_ERROR;
    }
    if (ngx_http_mogilefs_get_fid(source, fid) == NGX_ERROR) {
        return NGX_ERROR;
    }
    fid_len = ngx_strlen(fid);
    ngx_log_error(NGX_LOG_DEBUG, ngx_cycle->log, ngx_errno, "fid:%s\n", fid);
    etag_value = ngx_palloc(r->pool, fid_len+3);
    ngx_cpystrn(etag_value, (u_char *)"\"", 2);
    ngx_cpystrn(etag_value+1, fid, fid_len+1);
    ngx_cpystrn(etag_value+1+fid_len, (u_char *)"\"", 2);
    h = ngx_list_push(&r->headers_out.headers);
    if (h == NULL) {
        return NGX_ERROR;
    }
    h->hash = ngx_hash(ngx_hash(ngx_hash('E', 'T'), 'a'), 'g');
    ngx_str_set(&h->key, "ETag");
    h->value.len = fid_len+2;
    h->value.data = etag_value;
    h->lowcase_key = (u_char *) "etag";
...
static ngx_int_t 
ngx_http_mogilefs_del_input_header(ngx_http_request_t *r, const u_char *key)
{
    ngx_table_elt_t     *headers;
    ngx_uint_t          i;            
    size_t              key_len;
    ...
    headers = r->headers_in.headers.part.elts; 
    for (i=0; i<r->headers_in.headers.part.nelts; i++) {
        if (headers[i].key.len != key_len) {
            continue;
        }
        if (!ngx_strncmp(headers[i].key.data, key, key_len)) {
            for (;i<r->headers_in.headers.part.nelts; i++) {
